...
export const videogameRoutes = new Elysia({ prefix: "/videogames" })
	...
	.get("/genre/:genre", async ({ params }) => {
		try {
			const videogames = await Videogame.find({ genres: { $regex: params.genre, $options: "i" } }).lean();
	...
	.get("/theme/:theme", async ({ params }) => {
		try {
			const videogames = await Videogame.find({ themes: { $regex: params.theme, $options: "i" } });
			return { count: videogames.length, data: videogames };
		} catch {
	...
	.get("/language/:language", async ({ params }) => {
		try {
			const videogames = await Videogame.find({ languages: { $regex: params.language, $options: "i" } });
			return { count: videogames.length, data: videogames };
		} catch {
	...
	.get("/recent", async ({ query }) => {
		try {
			const limit = query.limit ?? 10;
			const videogames = await Videogame.find({ first_release_date: { $exists: true } })
