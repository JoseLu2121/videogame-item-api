		...
		try {
			...
			while (nextUrl) {
				...
				for (let i = 0; i < json.results.length; i += chunkSize) {
					...
					const detailedPromises = chunk.map(async (basic) => {
						...
						return {
							...
							tags: data.types.map((t) => t.type.name.toLowerCase()),
						};
					});
				...
				if (docs.length > 0) {
					...
					console.log(
						`      - Saved batch of ${docs.length} Pokemon... Total: ${totalSaved}`,
					);
				}
			...
			console.log(
				`   -> Saved ${totalSaved} total Pokemon characters via Mongoose.`,
			);
		} catch (e) {
