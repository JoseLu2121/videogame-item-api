export class XivapiSeeder {
    ...
    async run(db) {
        ...
        do {
            ...
            url.searchParams.set("fields", "Name,Description,Icon,LevelItem,ItemUICategory.Name,StackSize");
            if (after) url.searchParams.set("after", after);

            const res = await fetch(url.toString());
            if (!res.ok) throw new Error(`XIVAPI error: ${res.status} ${res.statusText}`);
        ...
        const itemsArray = allResults
            .filter((result) => result.fields.Name && result.fields.Name.trim() !== "")
            .map((result) => ({
