...
export class VideogameSeeder {
	...
	async run() {
		...
		const gameTypeMap = new Map(
			gameTypes.map((gameType) => [gameType.id, gameType]),
		);
		...
		const docs = videogames.map((videogame) => ({
			...
			collections: (videogame.collections ?? []).map(
				(collection) => collection.name,
			),
			franchises: (videogame.franchises ?? []).map(
			...
			languages: [
				...new Set(
					(videogame.language_supports ?? []).map(
						(languageSupport) =>
							languageMap.get(languageSupport.language)?.name,
					),
				),
			],
		}));
...
async function getIGDBGames({ clientId, accessToken }) {
  ...
  `;
	...
	if (!response.ok) {
		...
		throw new Error(
			`IGDB error: ${response.status} ${response.statusText} - ${errorBody}`,
		);
	}
...
async function getTwitchAccessToken({ clientId, clientSecret }) {
	...
	if (!response.ok) {
		const errorBody = await response.text();
		throw new Error(
			`Error requesting Twitch token: ${response.status} ${response.statusText} - ${errorBody}`,
		);
	}
