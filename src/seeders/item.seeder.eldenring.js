export class EldenRingItemsSeeder {
	...
	async run(db) {
		...
		while (hasMoreData) {
			...
			if (!response.ok) {
				throw new Error(
					`Error loading Elden Ring API on page ${page}: ${response.statusText}`,
				);
			}
			...
			if (json.data && json.data.length > 0) {
				...
				console.log(
					`      - Fetched page ${page} (${json.data.length} items)...`,
				);
				page++;
			} else {
				hasMoreData = false;
		...
		if (allItems.length > 0) {
			console.log(
				`   -> Saving a total of ${allItems.length} Elden Ring items in MongoDB...`,
			);
