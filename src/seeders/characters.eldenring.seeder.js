...
export class EldenRingCharactersSeeder {
  ...
  async run() {
    ...
    try {
      ...
      while (hasMore) {
        const response = await fetch(`https://eldenring.fanapis.com/api/npcs?limit=100&page=${page}`);
        ...
        const docs = json.data.map(npc => ({
          ...
          tags: npc.location ? [npc.location] : []
        }));

        await Character.insertMany(docs);
        totalSaved += docs.length;
        console.log(`      - Saved batch of ${docs.length} Elden Ring characters... Total: ${totalSaved}`);
        page++;
      }

      console.log(`   -> Saved ${totalSaved} total Elden Ring characters via Mongoose.`);
    } catch (e) {
